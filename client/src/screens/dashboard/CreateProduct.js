...
const CreateProduct = () => {
    ...
    const imageHandle = e => {
        if (e.target.files.length !== 0) {
            ...
            uploadAdsImage(e.target.files)
                .then((res) => {
                    console.log(res)
                    setState({ ...state, [e.target.name]: res });
                })
                .catch((e) => alert("error on firbase storage", e.message));
        }
    }
    const handleInput = e => {
                    ...
                    <div className="flex flex-wrap">
                        ...
                        <div className="w-full p-3">
                            ...
                            {sizes.length > 0 && <div className="flex flex-wrap -mx-3">
                                {sizes.map(size => (
                                    <div key={size.name} style={sizeList.some(item => item.name === size.name) ? { background: "white" } : {}} className='size' onClick={() => chooseSize(size)}>{size.name}</div>
                                ))}
                            </div>}
                        </div>
                        <div className="w-full p-3">
